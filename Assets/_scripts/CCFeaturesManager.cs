...
namespace CoffeeCommand {
	public class CCFeaturesManager : MonoBehaviour {
		...
		void GetCurrentFeatures(){
			...
			Vector3[] verts = CoffeeCommandFeaturesVisualizer.inst.GetComponent<MeshFilter> ().mesh.vertices;
			for(int i=0;i<verts.Length; i++){
				CurrentFeature cf = new CurrentFeature ();
				cf.point = CoffeeCommandFeaturesVisualizer.inst.transform.TransformPoint (verts[i]);
				cf.color = CoffeeCommandFeaturesVisualizer.inst.GetComponent<MeshFilter> ().mesh.colors [i];
